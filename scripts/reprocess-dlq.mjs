...
const resolveQueueUrls = (integration) => {
  const config = INTEGRATIONS[integration];
  const dlqUrl = process.env[config.dlqUrlEnv];
  const queueUrl = process.env[config.queueUrlEnv];
...
const buildAuditFilePath = ({ batchId, auditFile }) => {
  if (auditFile && auditFile.length > 0) {
    return path.resolve(process.cwd(), auditFile);
  }

  return path.resolve(process.cwd(), '.codex', 'runs', `dlq-reprocess-${batchId}.json`);
};
...
const run = async () => {
  ...
  for (const integration of integrations) {
    ...
    const { dlqUrl, queueUrl } = resolveQueueUrls(integration);
  ...
  process.stdout.write(`${JSON.stringify({ auditFilePath, summary }, null, 2)}\n`);
};
